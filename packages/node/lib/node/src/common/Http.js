"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __assign = (this && this.__assign) || function () {
    __assign = Object.assign || function(t) {
        for (var s, i = 1, n = arguments.length; i < n; i++) {
            s = arguments[i];
            for (var p in s) if (Object.prototype.hasOwnProperty.call(s, p))
                t[p] = s[p];
        }
        return t;
    };
    return __assign.apply(this, arguments);
};
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Http = void 0;
var https_1 = __importDefault(require("https"));
var form_data_1 = __importDefault(require("form-data"));
var querystring_1 = __importDefault(require("querystring"));
var lanzou_core_1 = require("lanzou-core");
var fs_1 = __importDefault(require("fs"));
var path_1 = __importDefault(require("path"));
function parseJson(str) {
    try {
        return JSON.parse(str);
    }
    catch (e) {
        return str;
    }
}
var Http = /** @class */ (function (_super) {
    __extends(Http, _super);
    function Http() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    Http.prototype.request = function (_a) {
        var _this = this;
        var headers = _a.headers, body = _a.body, url = _a.url, method = _a.method, onProgress = _a.onProgress;
        var event = new lanzou_core_1.Event();
        var promise = new Promise(function (resolve, reject) {
            var reqHeaders = __assign(__assign(__assign({}, lanzou_core_1.baseHeaders), { cookie: lanzou_core_1.common.auth.cookie }), headers);
            if (body instanceof form_data_1.default) {
                Object.assign(reqHeaders, body.getHeaders());
            }
            var req = https_1.default.request(url, { method: method, /*...options,*/ headers: reqHeaders }, function (res) {
                event.once('cancel', res.destroy);
                var buf = '';
                res.setEncoding('utf8');
                res.on('data', function (chunk) { return (buf += chunk); });
                res.on('end', function () {
                    return resolve({
                        json: function () { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
                            return [2 /*return*/, parseJson(buf)];
                        }); }); },
                        text: function () { return __awaiter(_this, void 0, void 0, function () { return __generator(this, function (_a) {
                            return [2 /*return*/, buf];
                        }); }); },
                        headers: res.headers,
                    });
                });
                res.on('error', reject);
            });
            event.once('cancel', function () {
                typeof req.abort === 'function' ? req.abort() : req.destroy();
                reject();
            });
            if (body instanceof form_data_1.default) {
                if (typeof onProgress === 'function') {
                    var bytes_1 = 0;
                    body.on('data', function (chunk) { return onProgress((bytes_1 += chunk.length)); });
                }
                ;
                body.pipe(req);
            }
            else {
                if (body)
                    req.write(typeof body === 'object' ? querystring_1.default.stringify(body) : body);
                req.end();
            }
        });
        promise.cancel = function () { return event.emit('cancel'); };
        return promise;
    };
    Http.prototype.download = function (options) {
        var event = new lanzou_core_1.Event();
        var promise = new Promise(function (resolve, reject) {
            https_1.default.get(options.url, function (res) {
                var _a, _b, _c, _d;
                var total = +res.headers['content-length'];
                var fileName = decodeURIComponent((_d = (_c = (_b = (_a = res.headers['content-disposition'].split(';')) === null || _a === void 0 ? void 0 : _a[1]) === null || _b === void 0 ? void 0 : _b.split('filename=')) === null || _c === void 0 ? void 0 : _c[1]) === null || _d === void 0 ? void 0 : _d.trim());
                // option.onStateChange?.({state: 3, length: total, disposition: fileName})
                fs_1.default.mkdirSync(path_1.default.dirname(options.path), { recursive: true });
                var file = fs_1.default.createWriteStream(options.path);
                event.once('cancel', function () {
                    res.destroy();
                    file.destroy();
                    reject();
                });
                // todo: 事件节流
                var len = 0;
                res.on('data', function (chunk) { var _a; return (_a = options.onProgress) === null || _a === void 0 ? void 0 : _a.call(options, (len += chunk.length), total); });
                res.on('end', function () {
                    file.end(); // 将内存中的内容全部写入，然后关闭文件
                    // option.onStateChange?.({state: 4})
                    resolve();
                });
                res.on('error', function () {
                    // option.onStateChange?.({state: 5})
                    reject();
                });
                res.pipe(file);
            });
        });
        promise.cancel = function () { return event.emit('cancel'); };
        return promise;
    };
    Http.prototype.upload = function (options) {
        var _this = this;
        var event = new lanzou_core_1.Event();
        var promise = new Promise(function (resolve, reject) {
            var fr = fs_1.default.createReadStream(options.path, options.end ? { start: options.start, end: options.end } : undefined);
            var form = new form_data_1.default();
            form.append('task', '1');
            form.append('ve', '2');
            form.append('lastModifiedDate', options.mtime.toString()); // 没有 toString 会报错
            form.append('type', 'application/octet-stream');
            form.append('id', "WU_FILE_0");
            form.append('folder_id_bb_n', options.folderId || -1);
            form.append('size', options.size);
            form.append('name', options.name);
            form.append('upload_file', fr, options.name);
            var handle = _this.request({
                url: "https://up.woozooo.com/fileup.php",
                method: 'post',
                body: form,
            });
            event.once('cancel', handle.cancel);
            handle
                .then(function (value) { return value.json(); })
                .then(function (response) {
                var _a, _b, _c;
                if (response.zt == 1 && ((_a = response.text) === null || _a === void 0 ? void 0 : _a.length))
                    console.log(response);
                resolve((_c = (_b = response.text) === null || _b === void 0 ? void 0 : _b[0]) === null || _c === void 0 ? void 0 : _c.id);
            })
                .catch(reject);
        });
        promise.cancel = function () { return event.emit('cancel'); };
        return promise;
    };
    return Http;
}(lanzou_core_1.HttpBase));
exports.Http = Http;
